import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

# Method 1 for SH calculation
def calculate_sh_method1(T, RH):
    SH = 38.015 * (10 ** ((7.65 * T) / (243.12 + T)) * RH) / (1013.25 - (0.06112 * 10 ** ((7.65 * T) / (243.12 + T)) * RH))
    return SH  # in g/kg

# Function to calculate the CDD
def calculate_cdd(df, T_base):
    CDD = np.zeros(len(df))
    for i in range(len(df)):
        Tmin = df.loc[i, 'Tmin[C]']
        Tmax = df.loc[i, 'Tmax[C]']
        Tavg = df.loc[i, 'Tavg[C]']
        
        if Tmax <= T_base:
            CDD[i] = 0
        elif Tavg <= T_base < Tmax:
            CDD[i] = (Tmax - T_base) / 4
        elif Tmin < T_base < Tavg:
            CDD[i] = ((Tmax - T_base) / 2) - ((T_base - Tmin) / 4)
        elif Tmin >= T_base:
            CDD[i] = Tavg - T_base
    
    return CDD

# Load the temperature data
df_trondheim = pd.read_csv('trondheim_temperature_SSP2-45.csv', delimiter=';')
df_rome = pd.read_csv('rome_temperature_SSP2-45.csv', delimiter=';')

# Preprocess the data
df_trondheim.columns = ['datetime', 'year', 'month', 'day', 'Tmin[K]', 'Tmax[K]']
df_rome.columns = ['datetime', 'year', 'month', 'day', 'Tmin[K]', 'Tmax[K]']

df_trondheim['Tmin[K]'] = df_trondheim['Tmin[K]'].str.replace(',', '.').astype(float)
df_trondheim['Tmax[K]'] = df_trondheim['Tmax[K]'].str.replace(',', '.').astype(float)
df_rome['Tmin[K]'] = df_rome['Tmin[K]'].str.replace(',', '.').astype(float)
df_rome['Tmax[K]'] = df_rome['Tmax[K]'].str.replace(',', '.').astype(float)

df_trondheim['Tmin[C]'] = df_trondheim['Tmin[K]'] - 273.15
df_trondheim['Tmax[C]'] = df_trondheim['Tmax[K]'] - 273.15
df_rome['Tmin[C]'] = df_rome['Tmin[K]'] - 273.15
df_rome['Tmax[C]'] = df_rome['Tmax[K]'] - 273.15

df_trondheim['Tavg[C]'] = (df_trondheim['Tmin[C]'] + df_trondheim['Tmax[C]']) / 2
df_rome['Tavg[C]'] = (df_rome['Tmin[C]'] + df_rome['Tmax[C]']) / 2

# Define RH values for each scenario
RH_good = 0.30
RH_average = 0.50
RH_bad = 0.70

# Calculate SH for each scenario
df_trondheim['SH_Good'] = calculate_sh_method1(df_trondheim['Tavg[C]'], RH_good)
df_trondheim['SH_Average'] = calculate_sh_method1(df_trondheim['Tavg[C]'], RH_average)
df_trondheim['SH_Bad'] = calculate_sh_method1(df_trondheim['Tavg[C]'], RH_bad)

df_rome['SH_Good'] = calculate_sh_method1(df_rome['Tavg[C]'], RH_good)
df_rome['SH_Average'] = calculate_sh_method1(df_rome['Tavg[C]'], RH_average)
df_rome['SH_Bad'] = calculate_sh_method1(df_rome['Tavg[C]'], RH_bad)

# Calculate CDD for each scenario
df_trondheim['CDD_Good'] = calculate_cdd(df_trondheim, 2)
df_trondheim['CDD_Average'] = calculate_cdd(df_trondheim, 7)
df_trondheim['CDD_Bad'] = calculate_cdd(df_trondheim, 15)

df_rome['CDD_Good'] = calculate_cdd(df_rome, 2)
df_rome['CDD_Average'] = calculate_cdd(df_rome, 7)
df_rome['CDD_Bad'] = calculate_cdd(df_rome, 15)

# Group by year for CDD and SH
CDD_good_trondheim = df_trondheim.groupby('year')['CDD_Good'].sum()
CDD_average_trondheim = df_trondheim.groupby('year')['CDD_Average'].sum()
CDD_bad_trondheim = df_trondheim.groupby('year')['CDD_Bad'].sum()

SH_good_trondheim = df_trondheim.groupby('year')['SH_Good'].sum()
SH_average_trondheim = df_trondheim.groupby('year')['SH_Average'].sum()
SH_bad_trondheim = df_trondheim.groupby('year')['SH_Bad'].sum()

CDD_good_rome = df_rome.groupby('year')['CDD_Good'].sum()
CDD_average_rome = df_rome.groupby('year')['CDD_Average'].sum()
CDD_bad_rome = df_rome.groupby('year')['CDD_Bad'].sum()

SH_good_rome = df_rome.groupby('year')['SH_Good'].sum()
SH_average_rome = df_rome.groupby('year')['SH_Average'].sum()
SH_bad_rome = df_rome.groupby('year')['SH_Bad'].sum()

# Define function to calculate energy demand
def calculate_ed(CDD, U_value):
    A_over_V = 1
    hours_per_year = 8760
    return (A_over_V * hours_per_year * U_value * CDD) / 1000  # kWh

# Define function to calculate humidity energy demand
def calculate_humidity_ed(SH, ACH=1.0, k=0.68, L_evap=2257, rho_air=1.225):
    hours_per_year = 8760
    return ACH * hours_per_year * k * L_evap * rho_air * (SH / 1000) / 1000  # Convert to kWh

# Calculate total energy demand over a specified period
def calculate_total_ed(CDD_trondheim, CDD_rome, SH_trondheim, SH_rome, start_year, end_year, U_value_trondheim=0.18, U_value_rome=0.32):
    # Thermal ED
    ED_therm_trondheim = calculate_ed(CDD_trondheim.loc[start_year:end_year].sum(), U_value_trondheim)
    ED_therm_rome = calculate_ed(CDD_rome.loc[start_year:end_year].sum(), U_value_rome)
    
    # Humidity ED
    ED_hum_trondheim = calculate_humidity_ed(SH_trondheim.loc[start_year:end_year].sum())
    ED_hum_rome = calculate_humidity_ed(SH_rome.loc[start_year:end_year].sum())
    
    return ED_therm_trondheim + ED_hum_trondheim, ED_therm_rome + ED_hum_rome

# Lists to store the total energy demand for each year in each scenario
years = list(range(2015, 2066))
ed_good_trondheim, ed_good_rome = [], []
ed_average_trondheim, ed_average_rome = [], []
ed_bad_trondheim, ed_bad_rome = [], []

# Calculate the total energy demand for each year
for year in years:
    ed_good = calculate_total_ed(CDD_good_trondheim, CDD_good_rome, SH_good_trondheim, SH_good_rome, year, year)
    ed_average = calculate_total_ed(CDD_average_trondheim, CDD_average_rome, SH_average_trondheim, SH_average_rome, year, year)
    ed_bad = calculate_total_ed(CDD_bad_trondheim, CDD_bad_rome, SH_bad_trondheim, SH_bad_rome, year, year)
    
    ed_good_trondheim.append(ed_good[0])
    ed_good_rome.append(ed_good[1])
    ed_average_trondheim.append(ed_average[0])
    ed_average_rome.append(ed_average[1])
    ed_bad_trondheim.append(ed_bad[0])
    ed_bad_rome.append(ed_bad[1])

# Plot the energy demand for each scenario over 50 years
plt.figure(figsize=(14, 10))

# Plot for Good scenario
plt.plot(years, ed_good_trondheim, label="Trondheim (Good)", color="green")
plt.plot(years, ed_good_rome, label="Rome (Good)", color="green", linestyle='--')

# Plot for Average scenario
plt.plot(years, ed_average_trondheim, label="Trondheim (Average)", color="black")
plt.plot(years, ed_average_rome, label="Rome (Average)", color="black", linestyle = '--')

# Plot for Bad scenario
plt.plot(years, ed_bad_trondheim, label="Trondheim (Bad)", color="red")
plt.plot(years, ed_bad_rome, label="Rome (Bad)", color="red", linestyle = '--')

plt.xlabel("Year")
plt.ylabel("Total Energy Demand (kWh)")
plt.title("Annual Total Energy Demand (Thermal + Humidity) from 2015 to 2065 - Method 1")
plt.axvline(x=2024, color='blue', linestyle='--', linewidth=1.5, label='Current Year 2024')
plt.legend()
plt.grid(True)
plt.show()
